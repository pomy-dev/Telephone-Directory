...
export async function insertUserProfile(userData) {
  ...
  try {
    const { data, error } = await supabase.rpc("insert_pomy_user", {
      payload: userData,
    });
...
export async function addFlyerItems(store, flyerItems) {
  ...
  const { data, error } = await supabase.rpc("save_pomy_flyer", {
    p_store: store.trim(),
    p_items: mappedItems,
  });
...
export async function submitVehicleComment(
  vehicleId,
  comment,
  suggestion = null,
  userNumber,
...
export async function submitGig(jobData) {
  try {
    const { data, error } = await supabase.rpc("create_pomy_gig", {
      ...
      p_images:
        (await uploadImages("piece-jobs", "gig-images", jobData?.photos)) || [],
...
export async function fetchOpenGigsCount() {
  const { data, error } = await supabase.rpc("count_open_gigs");
...
/**
 * Fetches a paginated and filtered list of workers from pomy_workers.
 * * @param {Object} filters - Search and filter parameters
 * @param {string} [filters.searchTerm] - Text search for name, bio, or skills
 * @param {string} [filters.skill] - Filter by specific skill or 'all'
...
export const fetchPomyWorkers = async ({
  ...
  try {
    ...
    // CRITICAL: Ensure we return something so 'result' isn't undefined
    return {
      workers: [],
      hasMore: false,
      lastVisible: null,
...
export async function applyForGig(formData) {
  try {
    ...
    const { data, error } = await supabase.rpc("apply_forpomy_gig", {
      ...
      p_attachments: uploadedAttachments.map((file) => ({
        url: file.url,
        name: file.name,
        type: file.type,
...
export async function getApprovedGigs(userEmail) {
  try {
    const { data, error } = await supabase.rpc("get_my_approved_gigs", {
      p_email: userEmail,
    });
...
export async function deleteMyApplication(appId, userEmail) {
  try {
    const { error } = await supabase.rpc("delete_my_gig_application", {
