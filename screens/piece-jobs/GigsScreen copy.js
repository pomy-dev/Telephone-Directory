...
//delay search when the use type in the search
//after the user stop after sometime we auto search without them pressing
...
const GigsScreen = ({ navigation }) => {
  ...
  const debouncedGigSearch = useDebounce(gigSearch, 500);
  const debouncedWorkerSearch = useDebounce(workerSearch, 500);
  ...
  }, [workerCategory, debouncedWorkerSearch]);
  ...
  const loadWorkers = async (isNewSearch = false) => {
    ...
    const result = await fetchPomyWorkers({
      searchTerm: debouncedWorkerSearch || null,
      skill: workerCategory.toLowerCase() === "all" ? "all" : workerCategory,
      isAvailable: true,
      location: null,
      pageSize: 20,
  ...
  const fetchLiveGigs = async (isLoadMore = false) => {
    ...
    const filters = {
      category: gigCategory === "all" ? "all" : gigCategory,
      searchTerm: debouncedGigSearch || null,
      status: "open",
