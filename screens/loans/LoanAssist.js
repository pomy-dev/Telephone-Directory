...
export default function FinancialHubScreen({ navigation }) {
  ...
  const renderLoanCard = ({ item }) => {
    ...
    return (
      ...
      >
        {activeTab === "Loans" && (
          <TouchableOpacity style={styles.checkbox} onPress={(e) => { e.stopPropagation(); toggleLoanSelection(item.id); }}>
            <Icons.Ionicons name={selected ? "checkbox" : "square-outline"} size={22} color={selected ? "#F59E0B" : "#9CA3AF"} />
          </TouchableOpacity>
        )}
    ...
    <>
      ...
      <View style={styles.headerRow}>
        <Text style={styles.sectionTitle}>
          {activeTab === "Loans" ? "All Loan Entities" : activeTab === "Insurance" ? "Insurance Providers" : "Investment Parties"}
        </Text>
  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.colors.background }]}>
      ...
      <SecondaryNav title="Financial Hub" />
...
const styles = StyleSheet.create({
  ...
  // Tab Bar
  tabBar: { flexDirection: "row", paddingVertical: 12, paddingHorizontal: 16, borderBottomWidth: 1, borderColor: "#E5E7EB" },
  ...
  loanCardSelected: { borderColor: "#F59E0B", backgroundColor: "#FFF8E1", shadowColor: "#F59E0B", shadowOpacity: 0.2, shadowRadius: 8, elevation: 4 },
